  ...
  if (!config.setupComplete) {
    ...
    safeStep('startScheduler', () => {
      const doStart = () => {
        ...
        try { scheduler.start(); } catch (e) { log('[App] scheduler.start error:', e.message, e.stack); }
      };

      if (popOk !== null) {
        popupManager.waitForReady(10000)
          .then(() => {
